...
LOG_FILE = LOGS_DIR / "backfill_2021.log"
...
def get_2021_fridays() -> list[date]:
    """Generate a list of dates for every Friday in 2021."""
    start_date = date(2021, 1, 1)  # First Friday of 2021
    end_date = date(2021, 12, 31)  # Last Friday of 2021
...
# ---------------------------------------------------------------------------
def run_backfill():
    fridays = get_2021_fridays()
    total_dates = len(fridays)
    logger.info(f"Starting backfill for {total_dates} Fridays in 2021.")
...
if __name__ == "__main__":
    logger.info("=" * 60)
    logger.info("Initializing 2021 Backfill Pipeline (Weekly Friday Jump)")
    logger.info("=" * 60)
